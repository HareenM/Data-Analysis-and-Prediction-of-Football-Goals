...
for i,shot in shots.iterrows():
    ...
    if (team_name==home_team_required):
        if goal:
            shotCircle=plt.Circle((x,pitchWidthY-y),circleSize,color="#6495ed")
        else:
            shotCircle=plt.Circle((x,pitchWidthY-y),circleSize,color="#6495ed")     
            shotCircle.set_alpha(.4)
    elif (team_name==away_team_required):
        if goal:
            shotCircle=plt.Circle((pitchLengthX-x,y),circleSize,color="#be0032") 
        else:
            shotCircle=plt.Circle((pitchLengthX-x,y),circleSize,color="#be0032")      
...
fig.savefig('d)Output/1-ShotMap.pdf', dpi=100) 
plt.show()