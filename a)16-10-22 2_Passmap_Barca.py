#PassMap_Barca of 16th October,2022
...
for i,thepass in passes.iterrows():
    if thepass['team_name']==home_team_required:
        ...
        passCircle=plt.Circle((x,pitchWidthY-y),2,color="#be0032")      
        passCircle.set_alpha(.2)   
        ax.add_patch(passCircle)
        dx=thepass['pass_end_location'][0]-x
        dy=thepass['pass_end_location'][1]-y

        passArrow=plt.Arrow(x,pitchWidthY-y,dx,-dy,width=0.6,color="#be0032")
        ax.add_patch(passArrow)

fig.set_size_inches(10, 7)
fig.savefig('a)Output/2-PassMap_Barca.png', dpi=100) 
plt.show()